...
import sys
import os
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '/media/volume/name1/home/exouser/medkaformer/dataset_ddsm/GMIC/')))

from src.utilities import tools
from torchvision.models.resnet import conv3x3
from collections import OrderedDict
from packaging.version import Version
# from src.modeling.ball_feature import *
...
class PostProcessingStandard(nn.Module):    # 生成激活图 1*1 conv
    """
    ...
    def forward(self, x_out):
        # print("x_out shape before gn_conv_last:", x_out.shape)
...
class MixStyle(nn.Module):
    ...
    def forward(self, x):
        ...
        # Mix 统计信息
        lambda_ = torch.distributions.Beta(self.alpha, self.alpha).sample((batch_size, 1, 1, 1)).to(x.device)
        mean_mixed = lambda_ * mean + (1 - lambda_) * mean_perm
        std_mixed = lambda_ * std + (1 - lambda_) * std_perm

        return std_mixed * (x - mean) / std + mean_mixed



class GlobalNetwork(AbstractMILUnit):  # 提取特征
    """
...
class TopTPercentAggregationFunction(AbstractMILUnit):
    ...
    def __init__(self, parameters, parent_module):
        ...
        self.percent_t = 0.02
        # print('self.percent_t is',self.percent_t)
        self.parent_module = parent_module
...
class RetrieveROIModule(AbstractMILUnit):
    ...
    def forward(self, x_original, cam_size, h_small):
        ...
        overlap_ratio = 0.9 # 转化为最大覆盖问题，数值低，重叠得大；【0-1】
        for _ in range(self.num_crops_per_class):
            ...
            current_images = current_images * (1 - overlap_ratio * mask)
...
class AttentionModule(AbstractMILUnit):
    ...
    def forward(self, h_crops):
        ...
        # y_crops = torch.softmax(self.parent_module.classifier_linear(z_weighted_avg), dim=1)
...
class OFU_2view(nn.Module):
    ...
    def register_embed_again(self, shape):
        b, _, h, w = shape
        uv = uv_grid(h, w).view(1, 2, h, w)
        grid = torch.cat([torch.sin(uv[:, :1]), torch.cos(uv[:, :1]), torch.sin(uv[:, 1:]), torch.cos(uv[:, 1:]),
                          torch.cos(uv[:, :1]) * torch.cos(uv[:, 1:])], dim=1)
        self.grid_mlo = grid.expand([b, -1, -1, -1])

        # return self.grid_mlo


    def register_embed_again2(self, shape):
        b, _, h, w = shape
        uv = uv_grid(h, w).view(1, 2, h, w)
        grid = torch.cat([torch.sin(uv[:, :1]), torch.cos(uv[:, :1]), torch.sin(uv[:, 1:]), torch.cos(uv[:, 1:]),
                          torch.cos(uv[:, :1]) * torch.cos(uv[:, 1:])], dim=1)
        self.grid_cc = grid.expand([b, -1, -1, -1])
    ...
    def fuse_features(self, y_mlo, y_cc):


        # 球面变换对齐
        # print(y_mlo.shape)
        # # self.grid_mlo = self.register_embed(y_mlo.shape)
        # # self.grid_cc = self.register_embed(y_cc.shape)

        # self.grid_mlo = self.register_embed_again(y_mlo.shape)
        # self.grid_cc = self.register_embed_again2(y_cc.shape)

               
        # aligned_mlo = spherical_grid_sample(y_mlo, self.grid_mlo)
        # aligned_cc = spherical_grid_sample(y_cc, self.grid_cc)
        
        # 融合特征
        fused = aligned_mlo + aligned_cc  # 简单加和，也可以用注意力机制
        return fused

    
    def forward(self, x_mlo, x_cc):

        original_size = x_mlo.size()  # 保存输入的原始大小

        x_mlo = self.pad(x_mlo)
        x_cc = self.pad(x_cc)

        # 上采样以确保分辨率一致
        x_mlo = F.interpolate(x_mlo, scale_factor=self.scale, mode='bilinear', align_corners=False)
        x_cc = F.interpolate(x_cc, scale_factor=self.scale, mode='bilinear', align_corners=False)

        x_mlo = x_mlo[:, :, self.scale:-self.scale, self.scale:-self.scale]
        x_cc = x_cc[:, :, self.scale:-self.scale, self.scale:-self.scale]


        # 调用 wraped_sample 分别对 MLO 和 CC 视图进行变换
        y_mlo = self.wraped_sample(x_mlo)
        y_cc = self.wraped_sample(x_cc)

        # 融合 MLO 和 CC 视图特征（如特征对齐或直接加权平均）
        y = self.fuse_features(y_mlo, y_cc)
