...
class TopTPercentAggregationFunction(AbstractMILUnit):
    ...
    def __init__(self, parameters, parent_module):
        ...
        self.percent_t = parameters.get("percent_t", 0.02)
        self.parent_module = parent_module
...
class RetrieveROIModule(AbstractMILUnit):
    ...
    def __init__(self, parameters, parent_module):
        ...
        self.overlap_ratio = parameters.get("overlap_ratio", 0.0)
    ...
    def forward(self, x_original, cam_size, h_small):
        ...
        normalize_images = (current_images - min_vals) / range_vals
        ...
        for _ in range(self.num_crops_per_class):
            ...
            current_images = current_images * (1 - self.overlap_ratio * mask)
...
def _get_value(key):
    try:
        ...
        raise KeyError(f"Unknown global option: {key}") from exc
...
def xy_grid(h, w, dim=0, device=None):
...
def uv_grid(h, w, dim=0, device=None):
    device = device or torch.device("cuda" if torch.cuda.is_available() else "cpu")
    if Version(torch.__version__) >= Version('1.10.0'):
        y, x = torch.meshgrid(torch.arange(h, device=device), torch.arange(w, device=device), indexing='ij')
    else:
        y, x = torch.meshgrid(torch.arange(h, device=device), torch.arange(w, device=device))
    u = (x.type(torch.float32) - (w - 1) / 2) / w * (2 * np.pi)
    v = -(y.type(torch.float32) - (h - 1) / 2) / h * np.pi
...
class OFU(nn.Module):
    ...
    def register_embed(self, shape, device):
        b, _, h, w = shape
        uv = uv_grid(h, w, device=device).view(1, 2, h, w)
    ...
    def wraped_sample(self, x):
        ...
        if self.ofu_grid is None or not self.ofu_grid.shape[0] == b or not tuple(self.ofu_grid.shape[-2:]) == tuple(x.shape[-2:]):
            self.register_embed(x.shape, x.device)

        if self.xy_grid is None or not tuple(self.xy_grid.shape[-2:]) == tuple(x.shape[-2:]):
            self.xy_grid = xy_grid(h, w, device=x.device).view(1, 2, h, w)
...
class OFU_2view(nn.Module):
    ...
    def register_embed_again(self, shape, device):
        b, _, h, w = shape
        uv = uv_grid(h, w, device=device).view(1, 2, h, w)
        grid = torch.cat([torch.sin(uv[:, :1]), torch.cos(uv[:, :1]), torch.sin(uv[:, 1:]), torch.cos(uv[:, 1:]),
                          torch.cos(uv[:, :1]) * torch.cos(uv[:, 1:])], dim=1)
    ...
    def register_embed_again2(self, shape, device):
        b, _, h, w = shape
        uv = uv_grid(h, w, device=device).view(1, 2, h, w)
        grid = torch.cat([torch.sin(uv[:, :1]), torch.cos(uv[:, :1]), torch.sin(uv[:, 1:]), torch.cos(uv[:, 1:]),
                          torch.cos(uv[:, :1]) * torch.cos(uv[:, 1:])], dim=1)
