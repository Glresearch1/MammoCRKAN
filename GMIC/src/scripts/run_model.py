...
from src.modeling import double_gmic as gmic
...
def visualize_example(input_img, saliency_maps, true_segs,
    ...
    alpha_green = matplotlib.colormaps.get_cmap('Greens')
    alpha_green._init()
    alpha_green._lut[:, -1] = alphas
    # alpha_red = matplotlib.colormaps.get_cmap('Reds')
    alpha_red = matplotlib.colormaps.get_cmap('BuPu')
    ...
    if malignant_seg is not None:
        ...
        cm.OrRd.set_under('k', alpha=0)
        # cm.BuPu.set_under('w', alpha=0)
        # cm.BuPu.set_under(color=(1, 1, 1, 0))
        
        
    #     # subfigure.imshow(malignant_seg, alpha=0.85, cmap=cm.OrRd, clim=[0.9, 1])
    #     subfigure.imshow(malignant_seg, alpha=0.85, cmap=cm.BuPu, clim=[0.9, 1])
    ...
    cm.YlOrBr.set_under('w', alpha=0)  # 设置下界为白色
    subfigure.imshow(crop_mask, alpha=0.7, cmap=cm.YlOrBr, clim=[0.9, 1])
...
def main():
    ...
    # parser.add_argument('--model-path', default="/media/volume/name1/home/exouser/medkaformer/dataset_ddsm/result_ckp/bestmodel_checkpoint.pth")
